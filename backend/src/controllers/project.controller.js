...
export const createProject = asyncHandler(async (req, res) => {
    ...
    if (!/^[a-zA-Z0-9_-]+$/.test(name)) {
        throw AppError.badRequest('Project name can only contain letters, numbers, underscores, and hyphens');
    }
    ...
    if (existingProjects && existingProjects.length > 0) {
        throw AppError.badRequest('Project with this name already exists');
    }
    ...
    const trackingId = 'trk_' + shortToken(12);
...
export const getShareReport = asyncHandler(async (req, res) => {
    ...
    if (!project) {
        throw AppError.notFound('Report not found or link is invalid');
    }
...
export const regenerateShareToken = asyncHandler(async (req, res) => {
    const { id } = req.params;
    const userId = req.user.id;
...
export const getProjectPages = asyncHandler(async (req, res) => {
    const { id } = req.params;
    const userId = req.user.id;
